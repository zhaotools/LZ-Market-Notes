...
已由用户提供并确认公众号公开合集。使用 `scripts/sync-wechat.mjs` 核验合集 ID、公众号名称、公开账号标识和 `__biz`，只抓取公开目录元数据，生成 `feed.xml`，再由 RSS 解析器写入 `data/articles.json`。保持正文不搬运，不保存后台 cookie；失败时保留上一版目录。人工导入继续用于维护摘要、分类与标签。
...
定时同步已确定为每天北京时间 09:17 和 21:17，并在 10:07 和 22:07 进行低频补偿重试；没有新内容时不提交或部署。YouTube 使用官方 RSS 最多最近 5 条有效内容，并排除 `youtubeExcludedVideoIds` 中已人工确认无效的直播占位页；Map 使用公开 global 快照；个人订阅号使用公开合集生成项目自有 RSS。仅在真实内容变化时持久化 `data/`、`feed.xml` 与生成页面；失败保留旧快照，不清空内容，也不保存登录凭据或公众号正文。
