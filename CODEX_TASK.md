...
定时同步已确定为每天北京时间 09:17 和 21:17，并在 10:07 进行一次早间低频补偿重试；没有新内容时不提交或部署。YouTube 使用官方 RSS 最多最近 5 条有效内容，并排除 `youtubeExcludedVideoIds` 中已人工确认无效的直播占位页；Map 使用公开 global 快照；个人订阅号通过 GitHub Actions Secret `WECHAT_FEED_URL` 配置可替换的第三方 RSS/Atom 连接器。仅在真实内容变化时持久化 `data/` 与生成页面；失败保留旧快照，不清空内容，也不保存 Feed 凭据或公众号正文。
