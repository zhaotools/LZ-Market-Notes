...
使用 scripts/sync-youtube.mjs，在受控环境中通过仓库 Secret 提供 YOUTUBE_API_KEY。优先调用 Data API 读取并核验最近 5 条公开视频，API 临时失败时回退到 RSS 和公开频道页；核对返回的频道确实对应 @lzmarketwatch，检查标题、封面和分类。
...
定时同步已确定为每天北京时间 09:17 和 21:17，并在 10:07 和 22:07 进行低频补偿重试；没有新内容时不提交或部署。YouTube 优先使用 Data API、失败时回退到官方 RSS 和公开频道页，最多保留最近 5 条有效内容，并排除 `youtubeExcludedVideoIds` 中已人工确认无效的直播占位页；Map 使用公开 global 快照；个人订阅号使用公开合集生成项目自有 RSS。三个来源必须独立执行，单一来源失败时保留旧快照并继续更新其他来源；仅在真实内容变化时持久化 `data/`、`feed.xml` 与生成页面，不清空内容，也不保存登录凭据或公众号正文。
